...
namespace StrandedDeep.Localization
{
    public class Log
    {
        ...
        public static void Write(string info, bool appendTrace = false)
        {
#if !DEBUG
            return ;
#endif

            using (StreamWriter streamWriter = new StreamWriter(gameLogPath, true))
            {
        ...
        {
            Log.gameLogPath = Init.PlguinBasePath.TrimEnd(new char[]
               {
                '/'
               }) + "/game_log.txt";
            UnityEngine.Debug.LogWarning("日志路径:" + gameLogPath);
